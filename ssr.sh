...
BBR_installation_status(){
	if [[ ! -e ${BBR_file} ]]; then
		...
		cd "${file}"
		if ! wget -N --no-check-certificate https://raw.githubusercontent.com/coke50307/ssr/master/bbr.sh; then
...
Install_Libsodium(){
	...
	if [[ ${release} == "centos" ]]; then
		...
		echo -e "${Info} 下载..."
		#範本"https://github.com/jedisct1/libsodium/releases/download/${Libsodiumr_ver}/libsodium-${Libsodiumr_ver}.tar.gz"
	...
	ldconfig
	cd .. && rm -rf libsodium-1.0.17.tar.gz && rm -rf libsodium-1.0.17
...
Update_Shell(){
	sh_new_ver=$(wget --no-check-certificate -qO- -t1 -T3 "https://raw.githubusercontent.com/coke50307/ssr/master/ssr.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1) && sh_new_type="github"
	[[ -z ${sh_new_ver} ]] && echo -e "${Error} 无法链接到 Github !" && exit 0
	if [[ -e "/etc/init.d/ssr" ]]; then
		rm -rf /etc/init.d/ssr
		Service_SSR
	fi
	wget -N --no-check-certificate "https://raw.githubusercontent.com/coke50307/ssr/master/ssr.sh" && chmod +x ssr.sh
...
echo -e "  ShadowsocksR 一键管理脚本 0+r維護 ${Red_font_prefix}[v${sh_ver}]${Font_color_suffix}
